...
public class SharedSqlite {
	 ...
	 * 
	 * @param context
	 *            A context object to create a new SharedSqlite class
	 */
	 ...
	 *             SharedSqlite.initialize(context) to create a new instance
	...
	/*
	 * Close the database in case all the pointers to it are 'gone' that is
	 * openCounter value is 0
	 */
	...
	/*
	 * A private method used to add a row to the table in case it does not
	 * exists Or update a row value in case it exist in the database
	 */
	private boolean addOrUpdate(String dataKey, String value) {
		...
		try {
			...
			if (getStringValue(dataKey, null) != null) {
				...
				rowId = db.update(DatabaseEntry.TABLE_COMMON_DATA, row,
						DatabaseEntry.COLUMN_DATA_KEY + "='" + dataKey + "'",
						null);
				if (rowId > -1) {
					pass = true;
				}
				...
				rowId = db.insertWithOnConflict(
						DatabaseEntry.TABLE_COMMON_DATA, null, row,
			...
			// row.put(DatabaseEntry.COLUMN_DATA_KEY, dataKey);
			// row.put(DatabaseEntry.COLUMN_DATA_VALUE, value);
			// rowId = db.insertWithOnConflict(DatabaseEntry.TABLE_COMMON_DATA,
			// null, row, SQLiteDatabase.CONFLICT_REPLACE);
			// if(rowId > -1) {
			// pass = true;
			// }
	 ...
	 * key-value and the key will be used to fetch the data later on
	 * 
	 * @param dataKey
	 *            The key to be used later to fetch the data
	 * @param value
	 ...
	 * 
	 * @param dataKey
	 *            The key to look a new value by
	 * 
	 * @param defaultValue
	 *            A default return value, if there is no mapping For the given
	 *            key this value will be returned
	 * 
	 * @return A String value associated with the given key or the defaultValue
	 *         in Case there is no such mapping
	 */
	public <K> String getStringValue(K dataKey, String defaultValue) {
		...
		try {
			...
			cursor = db.query(DatabaseEntry.TABLE_COMMON_DATA, null,
					...
					new String[] { String.valueOf(dataKey) }, null, null, null);
			if (cursor != null) {
				if (cursor.moveToFirst()) {
					columnIndex = cursor
							.getColumnIndex(DatabaseEntry.COLUMN_DATA_VALUE);
					if (columnIndex > -1) {
	 ...
	 *            key this value will be returned
	 * 
	 * @return A Integer value associated with the given key or the defaultValue
	 *         in Case there is no such mapping
	 */
	 ...
	 *            key this value will be returned
	 * 
	 * @return A Double value associated with the given key or the defaultValue
	 *         in Case there is no such mapping
	 */
	 ...
	 *            key this value will be returned
	 * 
	 * @return A Float value associated with the given key or the defaultValue
	 *         in Case there is no such mapping
	 */
	 ...
	 *            key this value will be returned
	 * 
	 * @return A Boolean value associated with the given key or the defaultValue
	 *         in Case there is no such mapping
	 */
	...
	/**
	 * A method to retrieve an array of strings by a key
	 * 
	 * @param dataKey
	 *            The key to look a new value by
	 * @return A String array of values associated with the given key or null in
	 *         Case there is no such mapping
	 */
	...
	public boolean reset() {
		...
		try {
			db = openDatabase();
			cursor = db.query(DatabaseEntry.TABLE_COMMON_DATA, null, null,
					null, null, null, null);
			totalRows = cursor.getCount();
			rowsDeleted = db
					.delete(DatabaseEntry.TABLE_COMMON_DATA, null, null);
